...
*NOTE*: Emacs 31+ includes a built-in, super-charged ~markdown-ts-mode~,
developed by this package's author together with Stéphane Marks, and
derived from this package. If you are using Emacs 31 or newer, use the
built-in mode instead.

This package is intended only for Emacs 29 and 30, and will refuse to
load on Emacs 31+.

A major mode for Emacs providing basic syntax highlighting for Markdown
